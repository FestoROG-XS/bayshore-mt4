...
app.all('*', (req, res) => {
    console.log(timestamp+` [WARNING] ${req.method} ${req.url} is unhandled`);
    res.status(200).end();
})
...
https.createServer({key, cert, minVersion: 'TLSv1', maxVersion:'TLSv1'}, app).listen(PORT_BNGI, '0.0.0.0', 511, () => {
